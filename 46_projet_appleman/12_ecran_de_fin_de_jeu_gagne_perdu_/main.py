...
FONT_FIN_JEU = pygame.font.SysFont(None, 80)
N_BONNES_GAGNER = 5
N_VIES = 3
...
def dessiner_ecran_fin(text):
    text_surf = FONT_FIN_JEU.render(text, True, 'red')
    fenetre.blit(text_surf, ((LARGEUR - text_surf.get_width())/2, (HAUTEUR - text_surf.get_height())/2))
...
def main(): 
    ...
    while executer:
        ...
        if n_bonnes >= N_BONNES_GAGNER:
            dessiner_ecran_fin("Vous avez Gagné!")
            break

        if n_mauvaises >= N_VIES:
            dessiner_ecran_fin("Vous avez Perdu!")
            break
