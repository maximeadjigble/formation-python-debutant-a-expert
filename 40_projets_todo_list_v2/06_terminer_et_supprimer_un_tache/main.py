...
def handler_label(event):
    _tache = event.widget.cget("text")
    entree.delete(0, tk.END)
    entree.insert(0, _tache)
...
def afficher_labels():
    ...
    for idx, tache, etat in taches.recuperer():
        ...
        label.bind("<Button-1>", handler_label)   
...
def handler_terminer():
    taches.terminer(entree.get())
    afficher_labels()

def handler_supprimer():
    taches.supprimer(entree.get())
    afficher_labels()
...
btn_supprimer = tk.Button(frame, command=handler_supprimer, text="-", padx=10)
btn_terminer = tk.Button(frame, command=handler_terminer, text="x", padx=10)
